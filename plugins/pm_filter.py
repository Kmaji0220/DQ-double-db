...
async def advantage_spoll_choker(bot, query):
    ...
    movies = SPELL_CHECK.get(query.reply_to_message)
    if not movies:
        return await query.answer(script.OLD_ALRT_TXT.format(query.from_user.first_name), show_alert=True)
    ...
    if gl == False:
        ...
        if k == False:
            files, offset, total_results = await get_search_results(query.message.reply_to_message, movie, offset=0, filter=True)
            if files:
